...
\begin{enumerate}
    ...
    \item \textbf{完整实验的验证}：基线模型（Dropout 0.25）在10 epochs训练中达到验证loss 3.6948，证明Dropout有效防止了过拟合。如果无Dropout，模型可能在训练集上达到更低loss，但验证loss会更高
...
\subsubsection{步骤1：运行消融实验（包含Baseline）}

消融实验脚本会自动训练基线模型和所有变体配置，生成完整的对比结果。

\begin{lstlisting}[language=bash, caption=运行消融实验（包含baseline训练）]
# Set random seed for reproducibility
export PYTHONHASHSEED=42

# Run all ablation experiments including baseline
CUDA_VISIBLE_DEVICES=1 python scripts/ablation_study.py \
    --epochs 10 \
    --run-all

# Expected output:
#   - Baseline checkpoint: results/checkpoints/ablation_Baseline.pth
#   - Ablation checkpoints: results/checkpoints/ablation_*.pth
#   - Training curves: results/figures/ablation_study.png
#   - Comparison chart: results/figures/ablation_comparison.png
#   - Estimated time: ~10-15 minutes on single GPU
\end{lstlisting}

\textbf{说明}：
\begin{itemize}
    \item \texttt{--run-all}：运行所有消融配置（Baseline、2 Heads、4 Heads、2 Layers、FFN 512、No Dropout、No Pos Encoding）
    \item \texttt{--epochs 10}：每个配置训练10个epoch
    \item 训练完成后，基线模型保存为\texttt{ablation\_Baseline.pth}
\end{itemize}

\subsubsection{步骤2：评估Baseline模型}

在Multi30k测试集上评估基线模型，获取BLEU分数和翻译样例。

\begin{lstlisting}[language=bash, caption=评估基线模型]
...
# Expected output:
#   - Validation loss: 3.6948
#   - BLEU Score: 5.99
#   - BLEU-1/2/3/4: 40.15 / 14.17 / 5.13 / 0.47
#   - Translation examples (10 samples)
#   - Estimated time: <1 minute
\end{lstlisting}

\textbf{说明}：
\begin{itemize}
    \item \texttt{--checkpoint}：指定评估的模型权重文件
    \item \texttt{--use-test-set}：使用测试集而非验证集
    \item \texttt{--verbose}：显示详细的翻译样例对比
\end{itemize}
